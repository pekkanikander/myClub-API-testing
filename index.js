...
const temp_black_group_id = 10429; // 9997; //
const temp_red_group_id   = 10430;
const temp_white_group_id = 10431;

const temp_ac_group_name = "06 AC"; // = 'Pojat-06';

const temp_tiago_black_event_id     = 414606; // = 382378; //
const temp_tiago_red_white_event_id = 414615; // = 382377; //
...
temp_tiago_red_white_event_promise.then(event => {
...
const create_monthly_invoices = function(bank_account, group, due_date) {
    return function(member_promise) {
        ...
        return Promise.all([bank_account, group, member_promise])
            .then(arr => {
                if (arr[2].member.external_license_id == null) {
                    // Not a player, skip
                    return null;
                }
                return new Invoice(arr[2],
 ...
 * @returns       nothing
 */
function temp_fill_invoice(invoice) {
    ...
    if (invoice._member.member.memberships.find(ms => ms.group_id == temp_black_group_id)) {
        ...
        // temp_maybe_add_no_tiago_line(invoice);
    }
...
function temp_invoicing(due_date, group_name) {
    ...
    const account = Members.account("FI5040550012095457");
    ...
    return iterate_members(group_name, create_monthly_invoice_for_member)
        .then(invoice_promise_array => {
            // console.log(invoice_promise_array);
            // Synchronise and wait for all invoices to become ready
            Promise.all(invoice_promise_array)
                .then(invoice_array => {
                    // Serialise POSTing of invoices
                    let promise = Promise.resolve();
                    invoice_array.filter(invoice => invoice != null).map(invoice => {
                        promise = promise.then(() => {
                            temp_fill_invoice(invoice);
                            return Members.post_invoice(invoice);
                        })
                    });
                    return promise;
                });
